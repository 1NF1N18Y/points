...
POINTSCLI=${POINTSCLI:-$SRCDIR/points-cli}
POINTSTX=${POINTSTX:-$SRCDIR/points-tx}
POINTSQT=${POINTSQT:-$SRCDIR/qt/points-qt}

[ ! -x $POINTSD ] && echo "$POINTSD not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
PNTVER=($($POINTSCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $POINTSD $POINTSCLI $POINTSTX $POINTSQT; do
  cmdname="${cmd##*/}"
