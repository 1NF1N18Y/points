...
The RPM spec file provided here is for Points-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://pntcrypto.com/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Points git repository](https://github.com/JustAResearcher/Points) but are not part of
...
Currently only `Source10` is used, for the example `points.conf` file.
...
in the points git repository. At present only an SVG version of the Points
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the points
...
needed when 0.12.1 is released, a proper fix is already in the Points git
master branch.
...
idea to build Points Core against a static version of that library compiled
...
The `points.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `points-server` package. At this point, most RPM based
...
The `points-server` package should have SELinux support. How to properly do
...
Points-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/points/). He has been
packaging Points for Fedora at least since 2012.
...
4. I renamed the `points` package that contains the Qt GUI to `points-core` as
that appears to be how the general population refers to it, in contrast to
`points-xt` or `points-classic`. I wanted to make sure the general population
