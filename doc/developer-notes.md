...
The Points Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play pointss" on the test network, if you
...
Points Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- PointsMiner : Generates points's (if wallet is enabled).
...
However, in open source software such as Points Core, where everyone uses
...
pay attention to for reviewers of Points Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Points Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Points Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Points Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/points-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/points-core/ctaes ; actively maintained by Core contributors.
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:points/points.git
