...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for PNT
    ...
    if (p_rewardSnapshot.strDistributionAsset != "PNT") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is PNT with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using PNT differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "PNT") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Points address (already validated during ownership snapshot creation)
