...
uint256 AWESOMEPOWHash(const CBlockHeader& blockHeader, uint256& mix_hash)
{
    ...
    uint256 nHeaderHash = blockHeader.GetAWESOMEPOWHeaderHash();
    const auto header_hash = to_hash256(nHeaderHash.GetHex());

    // ProgPow hash
    const auto result = awesomepow::hash(*context, blockHeader.nHeight, header_hash, blockHeader.nNonce64);
...
uint256 AWESOMEPOWHash_OnlyMix(const CBlockHeader& blockHeader)
{
    ...
    // ProgPow hash
    const auto result = awesomepow::hash_no_verify(blockHeader.nHeight, header_hash, to_hash256(blockHeader.mix_hash.GetHex()), blockHeader.nNonce64);
