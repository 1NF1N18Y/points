...
`test/functional/test_framework/test_framework.py:PointsTestFramework.main`.
...
def main():
    ...
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/JustAResearcher/Points/commit/d52802551752140cf41f0d9a225a43e84404d3e9
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    ...
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "pointsd"]) is not None:
