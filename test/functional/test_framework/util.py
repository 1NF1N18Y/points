...
def assert_fee_amount(fee, tx_size, fee_per_kb):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s PNT too low! (Should be %s PNT)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kb / 1000:
        raise AssertionError("Fee of %s PNT too high! (Should be %s PNT)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting PNT values"""
...
x16r_hash_cmd = os.path.dirname(os.path.realpath(__file__)) + "/../../../src/test/test_points_hash"
